...
async def get_tours_route(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener los tours: {str(e)}")
...
async def get_tour_route(tour_id: int, db: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener el tour: {str(e)}")
...
async def get_tour_with_services_route(tour_id: int, db: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener el tour con servicios: {str(e)}")
